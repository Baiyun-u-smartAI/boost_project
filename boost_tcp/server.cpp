...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "server")) {
        ...
        boost::asio::ip::tcp::acceptor acceptor(iosev,
                                                boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 1000));
        ...
        for (;;) {
            ...
            cv::Mat src = cv::imread(
                    R"(G:\mingw64\opencv\build\cpp_code_src\java\test\common_test\res\drawable\lena.png)");
            std::string message;
            ...
            std::cout << "已发送长度:" << message.length() << std::endl;
            // 如果出错，打印出错信息
            if (ec) {
                std::cout <<
